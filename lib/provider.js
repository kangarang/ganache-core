...
let Transaction = require("./utils/transaction");
...
Provider.prototype.send = function(payload, callback) {
  ...
  let intermediary = function(err, result) {
    ...
    if (payload.method === "eth_sendRawTransaction" && response.error.message.includes("base fee exceeds gas limit")) {
      let data;
      if (Buffer.isBuffer(payload.params[0])) {
        data = payload.params[0];
      } else if (payload.params[0] && payload.params[0].type === "Buffer") {
        data = to.buffer(payload.params[0].data);
      } else {
        console.log("huh?", payload);
      }
      const tx = new Transaction(data, Transaction.types.signed);
      response.error.message = response.error.message.concat(". GAS LIMIT PROVIDED: ", to.hex(tx.gasLimit).toString());
    }
